...
def draw_preview(image_rgb: np.ndarray, box_xyxy: np.ndarray, points) -> np.ndarray:
    """
    Dessine bbox + points FG/BG sur l'image (prévisualisation avant segmentation).
    FG = vert, BG = rouge
    """
    ...
    for (x, y, lab) in points:
        color = (0, 255, 0) if int(lab) == 1 else (0, 0, 255)
...
# 5) Prévisualisation bbox + points
...
# 6) Segmentation : bbox + points -> multimask
...
# 7) Choix du masque candidat + affichage + sauvegarde
lp = st.session_state["last_pred"]
if lp is not None and lp["img_name"] == img_name:
    masks = lp["masks"]
    scores = lp["scores"]
    dt = float(lp.get("time_ms", 0.0))
    ...
    st.write({
        ...
        "box_xyxy": [int(v) for v in lp["box"].tolist()],
    })
    ...
    st.image(
        overlay,
        caption=f"mask_idx={idx} | score={float(scores[idx]):.3f} | time={dt:.1f} ms",
