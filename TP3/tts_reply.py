...
def main():
    ...
    # Bon choix “gratuit HF”, stable et courant :
    ...
    # durée audio : gère [T] ou [C,T] ou [T,C]
    if audio.ndim == 1:
        audio_dur_s = float(audio.shape[0] / float(sr))
    elif audio.ndim == 2:
        # si [T, C] ou [C, T], on prend l’axe temps le plus grand
        audio_dur_s = float(max(audio.shape) / float(sr))
    else:
